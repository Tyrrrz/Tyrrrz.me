...
The default behavior makes sense as a safe choice, seeing as some syntax may depend on specific runtime capabilities to work correctly. However, it's a hindrance when developing libraries, which often need to target a wide range of frameworks to maximize compatibility. By setting the language version explicitly, we force the compiler to evaluate the availability of each language feature individually — immediately unlocking standalone features, while also allowing other features to be enabled with the help of polyfills (as we will see later).
