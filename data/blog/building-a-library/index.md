...
Therefore, in order to make use of NRT across the board and ensure the highest degree of compatibility, we configure the feature in its `annotations` mode as the baseline, and extend it to the `enable` mode for frameworks that fully support it. Doing so, our library will compile and provide its own nullability annotations for all targets, but we'll get warnings in our code only when building against newer frameworks.
...
Moving on, we set the `TreatWarningsAsErrors` property to `true`, directing the compiler to block the build if any warnings are encountered. This is basically the .NET's equivalent of "strict mode", which forces developers to address every potential issue in the codebase — either by fixing it or by explicitly declaring it as non-problematic. While it's not required, it's generally a good idea to enable this setting for library projects as they tend to have somewhat higher expectations when it comes to code quality.
...
The second group of properties is dedicated for toolchain-related options, which control the build process past the compilation stage. Here, we set the `IsPackable` property to `false`, which prevents the projects from being packed into NuGet packages when running `dotnet pack`. This inverts the default behavior, which assumes all projects to be packable unless explicitly marked otherwise.

In doing so, we create a more intentional workflow, where only projects that are explicitly marked as packable (i.e. the library and not the tests, samples, etc.) will be included in the packing process. As we'll explore later, this approach greatly simplifies the release workflow because we're able to coordinate the packing and publishing steps without having to worry about filtering out non-packable projects.
