...
Starting off with the compiler options, we set the `<LangVersion>` property to `latest`, instructing the C# (or F#, VB) compiler to use the latest stable version of the language. It's rather important to set this property explicitly, as its default value is not consistent and depends on the target framework that you're building for. This makes sense because some language features may rely on certain framework APIs to work properly, but when building a library, you're almost always going to target multiple frameworks at the same time, including older ones, so stepping into unsupported territory is the only way to gain access to the latest language features. Overall, this is not really a problem, and there are ways to retrofit the missing APIs, so setting `latest` here is perfectly safe.
