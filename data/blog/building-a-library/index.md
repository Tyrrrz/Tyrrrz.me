...
This is where community-driven polyfill libraries come into play. Unlike the `System.*` and `Microsoft.Bcl.*` packages, they don't constrain themselves to the most widely used types; they cover a much larger portion of the framework, aim to backport support for newer language features, and often leverage more flexible polyfilling techniques — including the ability to patch individual members through global extensions.

Additionally, many of such libraries are distributed as source code packages, which means that the polyfill implementations are injected directly into your project during compilation. This approach not only eliminates the run-time dependency on external assemblies, but also allows for more granular optimizations, such as inlining and dead code elimination, resulting in better overall performance.

There are quite a few options to choose from in this space, each with its own strengths and weaknesses, however the most popular ones are: [PolyShim](https://github.com/Tyrrrz/PolyShim), [Polyfill](https://github.com/SimonCropp/Polyfill), and [PolySharp](https://github.com/Sergio0694/PolySharp). These projects have somewhat different feature sets and design philosophies, but they ultimately have the same goal of plugging the gaps left by the official backport packages, providing polyfills to facilitate a broader range of usage scenarios.
