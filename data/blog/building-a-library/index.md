...
In the above snippet, we have a few different groups of properties that are used to configure various aspects of the build process. Each group is wrapped in a `<PropertyGroup>` element, which allows us to logically separate the properties based on their purpose. Doing so is not strictly necessary, but it helps keep things organized and makes reading and maintaining the file easier.
...
The default behavior is a sensible safeguard, seeing as certain language constructs may depend on specific runtime capabilities to work correctly. However, it’s impractical for developing libraries, as they typically need to target multiple frameworks for maximum compatibility. Setting the language version explicitly forces the compiler to evaluate the availability of each language feature independently from the target framework, immediately unlocking some of them and allowing others to be retrofitted with the help of polyfills.

Following that, we enable the [Nullable Reference Types](https://learn.microsoft.com/dotnet/csharp/nullable-references) feature of the C# compiler, as it is a great way to improve the safety of our code and to more accurately advertise the capabilities of our APIs to the consumers. There are two modes in which this feature can be configured: `annotations`, which instructs the compiler to emit nullability annotations for any of our own defined types and members; and `enable`, which also enables compiler warnings about potential violations of the nullability contracts.

While it's possible to use the `annotations` mode across all frameworks (with polyfills), the `enable` mode is only really useful when building against frameworks that have their own nullability information, which is the case starting with .NET Standard 2.1 and .NET Core 3.0. Therefore, in order to make use of NRTs across the board and ensure the highest degree of compatibility as well, we configure the feature in its `annotations` mode as the baseline, and extend it to the `enable` mode for frameworks that fully support it. Doing so, our library will compile and provide its own nullability annotations for all targets, but we'll only get compiler warnings about potential violations when building against newer frameworks.

Note how the example above relies on the `Condition="..."` attribute to validate framework compatibility. Instead of hard-codding a sequence of separate checks for each specific framework that our projects may target, we can use the [`IsTargetFrameworkCompatible`](https://learn.microsoft.com/visualstudio/msbuild/property-functions#msbuild-property-functions) MSBuild function to establish a lower boundary for the framework version, that also takes into account different flavors of .NET.

Moving on, we set the `TreatWarningsAsErrors` property to `true`, directing the compiler to block the build if any warnings are encountered. This is basically the .NET's equivalent of "strict mode", which forces developers to address every potential issue in the codebase — either by fixing it or by explicitly declaring it as non-problematic. While it's not required in any way, it's generally a good idea to enable this setting for library projects, as they tend to have somewhat higher expectations when it comes to code quality.

Rounding off the first section, we also set the `CheckEolTargetFramework` property to `false`, which disables warnings when building for frameworks that have exited their support lifecycle. As mentioned before, libraries do often need to target older frameworks for compatibility reasons, so these warnings are not particularly useful in this context.
