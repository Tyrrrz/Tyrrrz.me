...
The default behavior is a sensible safeguard, seeing as certain language constructs may depend on specific runtime capabilities to work correctly. However, library projects typically need to target multiple frameworks, including older ones, to maximize compatibility, making the aforementioned coupling impractical. Setting the language version explicitly forces the compiler to evaluate the availability of each language feature independently from the target framework, immediately unlocking some of them and allowing others to be retrofitted with the help of polyfills.
...
Rounding off the first section, we set the `TreatWarningsAsErrors` property to `true`, directing the compiler to block the build if any warnings are encountered. This is basically the .NET's equivalent of "strict mode", which forces developers to address every potential issue in the codebase — either by fixing it or by explicitly declaring it as non-problematic. While it's not required, it's generally a good idea to enable this setting for library projects as they tend to have somewhat higher expectations when it comes to code quality.

The second group of properties is dedicated for toolchain-related options, which control the build process past the compilation stage. Here, we set the `IsPackable` property to `false`, which prevents assemblies from being packed into NuGet packages when running `dotnet pack`. This inverts the default behavior, which assumes all projects to be packable unless explicitly marked otherwise.

Doing so allows us to blindly run `dotnet pack` followed by `dotnet nuget push` on the entire solution, without having to worry about filtering out non-packable projects, such as tests and samples. As we'll explore later, this setup greatly simplifies the release workflow and its automation.
