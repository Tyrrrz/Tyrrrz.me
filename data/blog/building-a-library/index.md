...
Some language features depend on certain runtime capabilities to work properly, so this coupling is understandable as a safety measure. However, it can also become a hindrance when developing library projects, seeing as they have to target a broad range of frameworks to maximize compatibility, instead of just the latest version like applications do.

By setting the language version explicitly, we force the compiler to evaluate the availability of each language feature individually. This allows syntax that doesn't rely on specific runtime capabilities to just work out of the box, while features that do can often be added through polyfills, as we’ll see later.
