...
The default behavior is a sensible safeguard, seeing as certain language constructs may depend on specific runtime capabilities to work correctly. However, it’s impractical for developing libraries, as they typically need to target a broad range of frameworks to maximize compatibility with their consumers. Setting the language version explicitly therefore forces the compiler to evaluate the availability of each language feature independently from the target framework, immediately unlocking some of them and allowing others to be retrofitted with the help of polyfills (as we'll see later).
