...
The default behavior is reasonable for application projects, as they typically target the latest framework version anyway, but it's a hindrance for library projects, which need to target a broad range of frameworks to maximize compatibility. By setting the language version explicitly, we force the compiler to evaluate the availability of each language feature individually — immediately unlocking syntax that doesn't depend on specific runtime capabilities, while also allowing retrofitting other features into older frameworks using polyfills (as we'll see later).
