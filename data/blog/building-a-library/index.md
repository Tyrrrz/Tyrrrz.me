...
Starting off with the first block of options, we set the `<LangVersion>` property to `latest`, instructing the C# (or F#, VB) compiler to use the most recent stable version of the language. By default, the language version is instead inferred from the target framework of the project, allowing newer language features only where they are officially supported. This behavior is fine for application projects, since they usually target the latest version of .NET anyway, but it's not practical for libraries, as they need to be compatible with a broad range of frameworks, including older ones. Setting the language version explicitly is therefore the only way to gain access to the newest language features in such scenarios, although some of them may still require a bit extra work (as we'll see later).
