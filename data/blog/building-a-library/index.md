...
Moving along, we also have `nuget.config` — a file that can be used to configure various settings related to the NuGet package manager, the most important being the sources from which it resolves dependencies. By default, NuGet relies on the official [NuGet.org](https://nuget.org) registry, but this behavior can be overridden by other instances of the `nuget.config` file that may be present in the environment. To ensure a consistent (and secure) developer experience, we can create a solution-level configuration file that explicitly defines the package sources we want to use.
