...
This coupling makes sense for certain language features that depend on specific framework APIs to work correctly, but for library projects — which often need to target a broad range of different frameworks, including older ones to maximize compatibility — it becomes a limiting factor. In such cases, specifying the language version explicitly forces the compiler to evaluate the availability of each language feature individually, and also allows us to retrofit some otherwise unavailable syntax with the help of polyfills.
