...
Finally, we have the third group of properties that are used to define common metadata that gets embedded into the compiled assemblies. The **`<Version>`** property is a bit special — it's used to distinguish different iterations of the same package, and it plays a crucial role in the NuGet package management system. For local development, we set it to a placeholder value of `0.0.0-dev`, which will be overridden with a proper version number during the release process.

The two other properties, **`<Company>`** and **`<Copyright>`**, are purely informational and get surfaced in various places, such as file properties and the NuGet package details. Make sure to replace them with your own name or organization, as they will be used later to attribute the authorship of the library.

Now, the settings we've defined inside `Directory.Build.props` so far are pretty universal. Regardless of the specifics of the library solution we're building, these properties provide a solid foundation that can be copied and reused across different projects.
