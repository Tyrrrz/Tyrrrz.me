...
Finally, we have the `Directory.Build.props` file, which lets us define arbitrary MSBuild properties that should be applied to all projects in the solution. When running the build, the tooling will automatically look for instances of this file (along with `Directory.Build.targets`, if available) by traversing the directory tree from the project location, including their contents as if they were imported directly by the project files themselves. That makes this file a great place to define cross-cutting settings that are common to all projects, such as assembly and package metadata, compiler options, build settings, and so on.

The file generated by `dotnet new buildprops` makes no assumptions about your intentions and simple starts off empty. Let's fix that:
