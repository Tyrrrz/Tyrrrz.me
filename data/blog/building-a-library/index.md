...
MSBuild, the engine behind .NET's build orchestration, provides a few [convention-based mechanisms](https://learn.microsoft.com/visualstudio/msbuild/customize-by-directory) that make it easier to customize its behavior for the entire codebase. One such mechanism is the `Directory.Build.props` file, which provides a common set of MSBuild properties that the tooling automatically discovers and imports into every project. This makes it a natural place for centralizing configuration such as compiler options, build settings, and various metadata.

Running `dotnet new` left us with a completely empty `Directory.Build.props` file. Let's take a look at how we can set it up for our library solution:
