...
Depending on your needs, you may choose to set the `version` property to a more specific version, or configure `rollForward` to be less permissive — either to declare a dependency on certain bug fixes introduced in later versions, or to protect against potential breaking changes that may come with future major updates. However, the approach above is a great starting point, and should work well for most projects out there.
