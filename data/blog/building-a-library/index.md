...
Here we have the `<packageSources>` section that defines feeds from which NuGet will fetch dependencies. It's a list-based setting, so we start with the `<clear />` element to remove any previously defined sources, and then add a single item named `nuget` that points to the NuGet.org catalog. With this in place, all projects in the solution will resolve packages from this location, regardless of any other sources that may be configured on the machine.

The following `<config>` section is used to specify various key-value settings, and in our case, we leverage it set the `defaultPushSource` key to match the package source specified earlier. This means that when we run the `dotnet nuget push` command to upload our own packages, it will automatically infer NuGet.org as the target registry without requiring any additional arguments.

### `Directory.Build.props`

Finally, we have the `Directory.Build.props` file, which lets us define arbitrary MSBuild properties that should be applied to all projects in the solution. When running the build, the tooling will automatically look for instances of this file (along with `Directory.Build.targets`, if available) by traversing the directory tree from the project location, including their contents as if they were imported directly by the project files themselves. That makes `Directory.Build.props` a great place to configure cross-cutting concerns that are common to all projects, such as assembly and package metadata, compiler options, build settings, and so on.

The file generated by `dotnet new` makes no assumptions about your intentions and simply starts off empty. Let's change that:
...
Here we have a few different groups of properties, so let's cover them in order. First off, we have the compiler options, which are used to configure the C# compiler behavior. The `LangVersion` property is set to `latest`, which means that the code will be compiled using the latest available version of the C# language. By default, this property is instead inferred from the target framework or the project, because some language features require the right framework APIs to support them. This default is chosen more as a precaution as Microsoft chose not to support combinations of older frameworks and newer language versions, but it still safe to override it.

Following that, we also enable the [Nullable Reference Types](https://learn.microsoft.com/dotnet/csharp/nullable-references) feature of the C# compiler, as it is a great way to improve the safety and reliability of your code. This option is disabled by default on older frameworks and enabled on some (but not all) of the newer ones, so it's a good idea to make things consistent. However, in order to properly support this feature, the framework itself needs to have the necessary annotations available, which is only the case starting with .NET Standard 2.1 and .NET Core 3.0. To ensure the most efficient level of compatibility, we enable the NRT feature in its `annotations` mode for all frameworks, and in its `enable` mode only for those that support it fully. This way, our library will compile and contain nullable annotation on all APIs we provide, but we will only get warnings about nullability violations when building for frameworks that support it.

To build upon the previous point, we also set the `TreatWarningsAsErrors` property to `true`, which means that any compiler warnings will be considered as errors and will fail the build. This forces us to address every potential issue in the codebase and explicitly decide whether it's a warning that we want to ignore or a genuine problem that needs to be fixed, reducing noise in the process. Combined with NRT, this will also make all nullability violations fail the build, which is a great way to ensure that the library is safe to use and doesn't contain any potential pitfalls.

You don't have to group each set of properties as shown above, this is just for better readability.
