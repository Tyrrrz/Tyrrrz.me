...
Next, we get to the **`IsPackable`** property, which controls whether a given project should be included in the packaging process. The default value is `true`, meaning that every project is treated as packable unless specified otherwise. By inverting the default, we establish a more intentional convention where NuGet packages are only created for projects that deliberately declare their intent to do so.

With this setup in place, we completely eliminate the need for filtering out non-library assemblies, such as those produced by the tests and sample projects. This allows us to blindly run `dotnet pack` followed by `dotnet nuget push **/*.nupkg` on the entire solution to generate and publish all relevant NuGet packages in one go. As we'll see later, this approach greatly simplifies the release workflow, as well as its automation.

Finally, we have the third group of properties that are used to define common metadata that gets embedded into the compiled assemblies. Most of these are purely informational, but the **`<Version>`** property is a bit special — it's used to distinguish different iterations of the same package, and it plays a crucial role in the NuGet package management system. For local development, we set it to a placeholder value of `0.0.0-dev`, which will be overridden with a proper version number during the release process.

The two other properties, **`<Company>`** and **`<Copyright>`**, are also used
