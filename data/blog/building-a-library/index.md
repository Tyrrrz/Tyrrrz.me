...
Finally, we have the `Directory.Build.props` file, which lets us define arbitrary MSBuild properties that should be applied to all projects in the solution. When running the build, the tooling automatically looks for this file (and `Directory.Build.targets`, if available) within the directory hierarchy, and implicitly includes its contents in each project specification. This convention makes `Directory.Build.props` a great place to configure common cross-cutting concerns, such as assembly and package metadata, compiler options, build settings, and so on.
...
Starting off with the first block of options, we set the `<LangVersion>` property to `latest`, instructing the C# (or F#, VB) compiler to use the most recent stable version of the language. By default, the language version is instead inferred from the target framework of the project, allowing newer language features only where they are officially supported. This behavior is fine if you intend to only target the latest version of .NET anyway — such as when building an application — but libraries typically need to support a broader range of frameworks, including older ones, for compatibility.

Setting the language version explicitly is therefore the only way to gain access to a consistent set of language features in a multi-targeted library project. Although some features may require specific base APIs to work properly, there are ways to get them to compile even when targeting older frameworks, as we'll see a bit later.
