...
Starting off with the first block of options, we set the `<LangVersion>` property to `latest`, instructing the C# (or F#, VB) compiler to use the most recent stable version of the language. By default, the language version is not a static value and is instead determined by the target framework — only allowing newer language features when building against frameworks that officially support them.

This coupling makes sense because certain language features may depend on specific framework APIs to work correctly. However, some features are purely syntactic or compile-time enhancements that don't carry such implications — so enabling a higher language version leaves it up to the compiler to determine which features are actually available, on an individual basis.

Furthermore, we can also use polyfills (as we'll see later) to provide implementations for the missing APIs in older frameworks, making it possible to use most of the latest language features without being constrained by the target framework. While this does lead us into "better know what you're doing" territory, this is the only way to gain access to latest language features when building library projects — seeing as they have to maximize compatibility by building for a broad range of targets, including some older ones as well.
