...
Here we have the `<packageSources>` section that defines the feeds from which NuGet will resolve package dependencies. This setting is a list, so we start with the `<clear />` element to remove any previously defined sources, and then add a single item named `nuget` that points to the NuGet.org catalog. This ensures that all projects in the solution will resolve packages from this location, regardless of any other sources that may be configured on the machine.
