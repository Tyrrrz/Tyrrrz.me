...
Alternatively, if the library targets .NET Standard, then the version of that standard must be supported by the project's framework. However, the rules for that are slightly more complex and you need to refer to the [compatibility table](https://dotnet.microsoft.com/platform/dotnet-standard#versions) to determine how the versions align with each other.
...
Even with multi-targeting, compatibility is still a compromise, and there's no universal answer to which frameworks you should support. That said, here are some general guidelines that I follow when deciding what to target in my libraries:
