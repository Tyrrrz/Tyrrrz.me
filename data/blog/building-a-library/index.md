...
As explained in the auto-generated comment above, the `<clear />` element is used to remove any previously defined package sources, effectively resetting the list to a blank slate. Following that, the `<add />` element is used to re-define the official NuGet registry as the only source for package dependencies. Now, regardless of the machine on which the build takes place, `dotnet restore` will always resolve packages from the same source.

Finally, the `Directory.Build.props` file is used to define custom MSBuild properties that should be applied to all projects in the solution. This file is particularly useful for configuring cross-cutting concerns that are common between all projects, such as assembly metadata, compiler options, and build settings. The file generated by `dotnet new buildprops` starts off empty, so let's add a few properties to it:
