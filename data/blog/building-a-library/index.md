...
Starting off with the compiler options, we set the `<LangVersion>` property to `latest`, instructing the C# (or F#, VB) compiler to use the most recent stable version of the language. By default, this value is determined by the current target framework of the project, reserving the use of newer language versions only for the frameworks where they are officially supported. That is completely fine for application projects, as you normally want them to target the latest version of .NET anyway, but it's not practical when it comes to libraries, which typically build for a broad range of frameworks to maximize compatibility. Setting the language version explicitly is therefore the only way to gain access to the newest language features in such scenarios, even though it may take a bit more effort to get them working (as we'll see later).
